...
def transform_data(df):
    ...
    df = df.withColumn("condition_at", array_contains(col("conditions"), "@").cast("int"))
    df = df.withColumn("condition_i", array_contains(col("conditions"), "I").cast("int"))
    df = df.withColumn("condition_t", array_contains(col("conditions"),"T").cast("int"))

    window_spec = Window.partitionBy("symbol", "event_date") \
...
def main(symbol, event_date, input_path, output_path):
    ...
    input_full_path = f"{input_path}/{symbol}/event_date={event_date}/*.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_path", type=str, default="s3a://raw//ALPACA_API", help="Input S3 path")
    parser.add_argument("--output_path", type=str, default="s3a://trusted/ALPACA_DATA", help="Output S3 path")
